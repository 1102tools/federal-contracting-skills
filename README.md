...
| [GSA CALC+ Ceiling Rates](skills/gsa-calc-ceilingrates) | No key | Awarded NTE hourly rates from GSA MAS contracts (230K+ records). Independently tested across 8 runs on 2 Claude models (112 of 112 assertions passed; Rounds 2 and 3 patches shipped). [Testing Record](skills/gsa-calc-ceilingrates/TESTING.md). Reference content merged into main skill. |
| [BLS OEWS Wages](skills/bls-oews-api) | BLS key | Market wage data covering ~830 occupations across 530+ metro areas. Independently tested across 16 runs in two waves and 2 Claude models (112 of 112 assertions passed; Rounds 2 through 5 patches shipped; Dayton MSA renumbering + probe series verified live; SOC rollup fallback and 503 resilience added Round 5). [Testing Record](skills/bls-oews-api/TESTING.md). Reference content merged into main skill. |
| [GSA Per Diem Rates](skills/gsa-perdiem-rates) | api.data.gov | Federal travel per diem (lodging + M&IE) for all CONUS locations. FY-not-yet-published fallback rule shipped April 2026. Reference content merged into main skill. |
...
| [IGCE Builder: LH/T&M](skills/igce-builder-lh-tm) | No key* | BLS OEWS, GSA CALC+, GSA Per Diem | Labor Hour and T&M IGCEs with burden multiplier pricing. Independently tested across 3 runs on Claude Opus 4.7 (39 of 42 assertions passed on Opus; Round 1 patches shipped). [Testing Record](skills/igce-builder-lh-tm/TESTING.md). |
...
Want more consistent results across runs? Install the MCP servers. Eight MCP servers are published on PyPI covering five procurement data sources (USASpending, GSA CALC+, BLS OEWS, GSA Per Diem, SAM.gov) and three regulatory / policy tracking sources (eCFR, Federal Register, Regulations.gov). When installed in Claude Desktop or another MCP client, orchestration skills call these servers directly for deterministic tool calls instead of letting Claude generate API request code through SKILL.md instructions alone.
...
| [IGCE Builder: LH/T&M](skills/igce-builder-lh-tm) | 3 runs (1 wave) | Opus 4.7 | 39 of 42 passed on Opus, Round 1 patches shipped | [Testing Record](skills/igce-builder-lh-tm/TESTING.md) |
